...
type SyncPropertyRegistrySuite struct {
	suite.Suite
	Name   string
	Start  time.Time
...
func TestSyncPropertyRegistrySuite(t *testing.T) {
	suite.Run(t, new(SyncPropertyRegistrySuite))
}

// *****************************************************************************
// Suite Tools

func (suite *SyncPropertyRegistrySuite) SetupSuite() {
	var err error
...
func (suite *SyncPropertyRegistrySuite) TearDownSuite() {
	if suite.T().Failed() {
...
func (suite *SyncPropertyRegistrySuite) BeforeTest(suiteName, testName string) {
	suite.Logger.Infof("Test Start: %s %s", testName, strings.Repeat("-", 80-13-len(testName)))
	suite.Start = time.Now()
}

func (suite *SyncPropertyRegistrySuite) AfterTest(suiteName, testName string) {
	duration := time.Since(suite.Start)
	suite.Logger.Record("duration", duration.String()).Infof("Test End: %s %s", testName, strings.Repeat("-", 80-11-len(testName)))
}

func (suite *SyncPropertyRegistrySuite) LoadTestData(filename string) []byte {
...
func ExampleSyncPropertyRegistry_Unmarshal() {
	registry := argo.NewSyncPropertyRegistry[Something]().Add(Something1{}, Something2{})
...
func (suite *SyncPropertyRegistrySuite) TestCanCreateSyncPropertyRegistry() {
	registry := argo.NewSyncPropertyRegistry[Something]()
	suite.Assert().NotNil(registry)
}

func (suite *SyncPropertyRegistrySuite) TestCanAddTypes() {
	// Here we do not use NewSyncPropertyRegistry, to test stuff works with default Go constructor
	registry := argo.SyncPropertyRegistry[Something]{}
...
func (suite *SyncPropertyRegistrySuite) TestShouldFailWithoutProperty() {
	registry := argo.NewSyncPropertyRegistry[Something]().Add(Something1{}).Append(Something2{})
...
func (suite *SyncPropertyRegistrySuite) TestShouldFailWithInvalidJSON() {
	registry := argo.NewSyncPropertyRegistry[Something]().Add(Something1{}).Append(Something2{})
...
func (suite *SyncPropertyRegistrySuite) TestShouldFailWithInvalidData() {
	registry := argo.NewSyncPropertyRegistry[Something]().Add(Something1{}).Append(Something2{})
